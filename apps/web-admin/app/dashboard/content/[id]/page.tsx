...
export default function ContentDetailPage() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      {/* File download — new signed URL every page load */}
      {item.downloadUrl ? (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Download link expires in 2 hours · Contains DRAFT watermark until approved
              </p>
            </div>
            <a
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 mb-6 text-sm text-slate-400">
      ...
      {item.content && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-slate-700 mb-2">Generation Details</h2>
          <div className="bg-slate-50 rounded-lg px-4 py-3 text-xs text-slate-600 font-mono">
            <pre className="whitespace-pre-wrap break-all">{JSON.stringify(item.content, null, 2)}</pre>
          </div>
        </div>
      )}
