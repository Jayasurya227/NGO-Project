  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-700 mt-0.5">Select records and delete them</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-x-auto">
        {loading ? (
          <div className="p-8 text-center text-gray-700 text-sm">Loading...</div>
        ) : rows.length === 0 ? (
          <div className="p-8 text-center text-gray-700 text-sm">No records found</div>
        ) : (
          <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-gray-50">
              <tr>
                ...
                {tab === 'donors' ? (
                  <>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Org Name</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Type</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">KYC Status</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Created</th>
                  </>
                ) : (
                  <>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">NGO ID</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Title</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Sector</th>
                ...
                <tr key={row.id}
                  ...
                  {tab === 'donors' ? (
                    <>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{row.kycStatus}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{new Date(row.createdAt).toLocaleDateString()}</td>
                    </>
                  ) : (
                    <>
                      <td className="px-4 py-3 text-sm font-mono text-gray-700">{row.ngoId ?? '—'}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{row.title}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.sector?.replace(/_/g, ' ')}</td>
