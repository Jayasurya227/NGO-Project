        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Focused Area</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredInitiatives.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-6 py-4 text-center text-gray-500">No initiatives found</td>
              </tr>
            ) : (
              filteredInitiatives.map((initiative) => (
                <tr key={initiative.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-xs font-mono font-semibold text-indigo-700 bg-indigo-50 border border-indigo-200 px-2 py-1 rounded">
                      {getDisplayId(initiative)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900">{initiative.title}</div>
                    <div className="text-sm text-gray-500 truncate max-w-[200px]">
                      {initiative.description?.startsWith('%PDF') || initiative.description?.includes('endobj') ? 'Uploaded via document' : initiative.description}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onClick={() => router.push(`/dashboard/initiatives/${initiative.id}`)} className="text-blue-600 hover:text-blue-900 mr-4">View</button>
                    <button onClick={() => router.push(`/dashboard/initiatives/${initiative.id}`)} className="text-green-600 hover:text-green-900 font-medium mr-4">Milestones</button>
                    <button onClick={() => handleDeleteInitiative(initiative.id)} className="text-red-600 hover:text-red-900 font-medium">Delete</button>
                  </td>
                </tr>
              ))
            )}
