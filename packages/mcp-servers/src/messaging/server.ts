...
function registerTools(server: McpServer) {
  server.tool(
    ...
    "Verify a donor has not opted out before sending any communication",
    {
      donorId: z.string(),
    ...
    async ({ donorId, channel }) => {
      const donor = await prisma.donor.findUniqueOrThrow({
        where: { id: donorId },
        select: { notificationPrefs: true, unsubscribedAt: true },
      });
      ...
      return {
        content: [{
          type: "text",
          text: JSON.stringify({ eligible, reason: eligible ? null : "OPTED_OUT" }),
        }],
      };
    }
  );
  ...
  server.tool(
    "send_email",
    "Send an email to a donor (STUB — logs only until Phase 3)",
    {
      donorId: z.string(),
      subject: z.string(),
      htmlBody: z.string(),
      logId: z.string().describe("The CommunicationLog row ID"),
    },
  ...
  server.tool(
    "send_whatsapp",
    "Send a WhatsApp message to a donor (STUB — logs only until Phase 3)",
    {
      donorId: z.string(),
