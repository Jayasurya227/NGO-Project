...
function registerTools(server: McpServer) {
  server.tool(
    "compute_phash",
    "Compute the perceptual hash of an image file from S3",
    {
      s3Key: z.string().describe("The S3 key of the image file"),
      hashSize: z.number().default(16).describe("Hash grid size"),
    },
    async ({ s3Key, hashSize }) => {
      const buffer = await downloadFile(s3Key);
      const hash = await imghash.hash(buffer, hashSize);
  ...
  server.tool(
    ...
    "Compare a pHash against all existing evidence in the tenant to detect duplicates",
    {
      tenantId: z.string(),
      pHash: z.string(),
      threshold: z.number().default(10).describe("Hamming distance threshold"),
    },
    async ({ tenantId, pHash, threshold }) => {
      const allEvidence = await prisma.evidence.findMany({
        where: {
          pHash: { not: null },
          milestone: { initiative: { tenantId } },
        },
      ...
      return {
        content: [{
          type: "text",
          text: JSON.stringify({ hasDuplicates: duplicates.length > 0, duplicates }),
        }],
      };
    }
  );
  ...
  server.tool(
    "check_gps_distance",
    "Calculate distance between submission GPS and initiative site location",
    {
      submittedLat: z.number(),
      submittedLng: z.number(),
      initiativeLat: z.number(),
      initiativeLng: z.number(),
      thresholdKm: z.number().default(5).describe("Flag if distance exceeds this value"),
    },
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            exceedsThreshold: distKm > thresholdKm,
            flag: distKm > thresholdKm ? `GPS_MISMATCH:${distKm.toFixed(1)}km` : null,
          }),
        }],
      };
    }
