...
function registerTools(server: McpServer) {
  server.tool("find_similar_initiatives", "Find similar initiatives using vector search", {
    tenantId: z.string(),
    ...
    const results = await prisma.$queryRaw<Array<{ id: string; title: string; sector: string; cosine_similarity: number }>>`
  ...
  server.tool("check_initiative_has_embedding", "Check if initiative has embedding", {
    ...
    `;
    return { content: [{ type: "text", text: JSON.stringify({ hasEmbedding: result[0]?.has_embedding ?? false }) }] };
  });
}
